...
class SignalClassifier {
  ...
  async classifyWithAI(activity, goalContext = null) {
    ...
    } else if (activity.goalIds && activity.goalIds.length > 0) {
      // Handle multiple goals context
      goalInfo = `
    OBJETIVOS RELACIONADOS: ${activity.goalIds.length} objetivos selecionados
    ANÁLISE: Avalie se a atividade contribui significativamente para os objetivos selecionados.`;
    }
    ...
    ${activity.goalIds && activity.goalIds.length > 1 ? 'DICA: Atividades que contribuem para múltiplos objetivos tendem a ter maior valor estratégico.' : ''}
